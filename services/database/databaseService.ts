...
class DatabaseService {
    ...
    private async checkAndMigrateSchema(): Promise<void> {
        ...
        try {
            ...
            if (currentVersion < latestVersion) {
                ...
                // Ensure purchase bill items columns exist (for received_quantity)
                this.ensurePurchaseBillItemsColumnsExist();
    ...
    /**
     * Ensure purchase_bill_items table has the received_quantity column
     */
    ensurePurchaseBillItemsColumnsExist(): void {
        if (!this.db || !this.isInitialized) return;
        
        try {
            // Check if purchase_bill_items table exists
            const tableExists = this.query<{ name: string }>(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='purchase_bill_items'"
            ).length > 0;
            
            if (tableExists) {
                // Check existing columns
                const columns = this.query<{ name: string }>('PRAGMA table_info(purchase_bill_items)');
                const columnNames = new Set(columns.map(col => col.name));
                
                // Add received_quantity column if missing
                if (!columnNames.has('received_quantity')) {
                    console.log('🔄 Adding received_quantity column to purchase_bill_items table...');
                    this.execute('ALTER TABLE purchase_bill_items ADD COLUMN received_quantity REAL NOT NULL DEFAULT 0');
                }
            }
        } catch (error) {
            console.error('❌ Error ensuring purchase_bill_items columns exist:', error);
        }
    }
