...
export class WebSocketClient {
  ...
  private setupEventHandlers(): void {
    ...
    this.socket.on('data:updated', (data: any) => {
      ...
      this.handleEvent('data:updated', data);
    });

    // P2P specific events
    this.socket.on('purchase_order:created', (data: any) => {
      logger.logCategory('websocket', 'PO created:', data);
      this.handleEvent('purchase_order:created', data);
    });

    this.socket.on('purchase_order:updated', (data: any) => {
      logger.logCategory('websocket', 'PO updated:', data);
      this.handleEvent('purchase_order:updated', data);
    });

    this.socket.on('p2p_invoice:created', (data: any) => {
      logger.logCategory('websocket', 'P2P Invoice created:', data);
      this.handleEvent('p2p_invoice:created', data);
    });

    this.socket.on('p2p_invoice:updated', (data: any) => {
      logger.logCategory('websocket', 'P2P Invoice updated:', data);
      this.handleEvent('p2p_invoice:updated', data);
    });

    // Installment Plan events
    this.socket.on('installment_plan:created', (data: any) => {
      this.handleEvent('installment_plan:created', data);
    });

    this.socket.on('installment_plan:updated', (data: any) => {
      this.handleEvent('installment_plan:updated', data);
    });

    this.socket.on('installment_plan:deleted', (data: any) => {
      this.handleEvent('installment_plan:deleted', data);
    });

    // Plan Amenity events
    this.socket.on('plan_amenity:created', (data: any) => {
      this.handleEvent('plan_amenity:created', data);
    });

    this.socket.on('plan_amenity:updated', (data: any) => {
      this.handleEvent('plan_amenity:updated', data);
    });
