/**
 * AUTO-GENERATED — do not edit. Source: shared/contract-retention/contractRetentionCore.ts
 * Regenerate: node scripts/ensure-shared-financial-cores.mjs
 */

import {
  BLOCK_PAYMENTS_ABOVE_RETENTION_LIMIT,
  RETENTION_WARNING_RATIO,
  type ContractRetentionFields,
  type RetentionAlertLevel,
  type RetentionSummary,
  type RetentionThresholdValidation,
  type RetentionType,
} from './types.js';
...
export function normalizeRetentionType(raw: unknown): RetentionType {
...
export function calculateRetentionAmount(
  ...
  fields: Pick<ContractRetentionFields, 'retentionType' | 'retentionPercentage' | 'retentionAmount'>
): number {
...
export function calculateMaximumPayable(contractValue: number, retentionAmount: number): number {
  const value = roundMoney(Math.max(0, contractValue));
  const retention = roundMoney(Math.max(0, retentionAmount));
  return roundMoney(Math.max(0, value - retention));
}

export function resolveRetentionAlertLevel(
  paidAmount: number,
  maximumPayable: number,
  hasRetention: boolean
): RetentionAlertLevel {
  if (!hasRetention || maximumPayable <= 0) return 'none';
  const paid = roundMoney(Math.max(0, paidAmount));
  const max = roundMoney(maximumPayable);
  if (paid >= max - MONEY_EPS) return 'critical';
  if (paid >= roundMoney(max * RETENTION_WARNING_RATIO) - MONEY_EPS) return 'warning';
  return 'none';
}
...
export function validateRetentionThreshold(input: {
  ...
  const projectedPaid = roundMoney(
    Math.max(0, input.projectedPaidAmount ?? input.paidAmount)
  );
  ...
  if (summary.alertLevel === 'critical') {
    return {
      ...
      message:
        `Payments have reached the maximum payable amount excluding retention.\n\n` +
...
export function shouldBlockPaymentAboveRetentionLimit(validation: RetentionThresholdValidation): boolean {
  if (!BLOCK_PAYMENTS_ABOVE_RETENTION_LIMIT) return false;
  return validation.alertLevel === 'critical';
}

export function computeRetentionBalanceOnSave(
  contractValue: number,
  fields: ContractRetentionFields
): { retentionBalance: number; retentionReleased: number } {
