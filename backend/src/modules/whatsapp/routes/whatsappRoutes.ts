 ...
 */
whatsappRouter.get('/config', async (req: AuthedRequest, res) => {
  try {
    if (!req.tenantId) {
 ...
 */
whatsappRouter.post('/config', async (req: AuthedRequest, res) => {
  try {
    if (!req.tenantId) {
      console.error('[WhatsApp Config] Missing tenantId in request');
      return res.status(401).json({
 ...
 */
whatsappRouter.delete('/config', async (req: AuthedRequest, res) => {
  try {
    const whatsappService = getWhatsAppApiService();
 ...
 */
whatsappRouter.post('/test-connection', async (req: AuthedRequest, res) => {
 ...
 */
whatsappRouter.post('/send', async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    if (!req.tenantId) {
      console.error(`[WhatsApp Send] [${requestId}] ﯔ MISSING TENANT ID - Request blocked`, {
    ...
    if (!phoneNumber || !message) {
      console.error(`[WhatsApp Send] [${requestId}] ﯔ Missing required fields`, {
    ...
    if (DEBUG_WHATSAPP) console.log(`[WhatsApp Send] [${requestId}]  Fields validated, calling sendTextMessage service`, {
      tenantId: req.tenantId,
    ...
    const duration = Date.now() - startTime;
    if (DEBUG_WHATSAPP) console.log(`[WhatsApp Send] [${requestId}]  MESSAGE SENT SUCCESSFULLY `, {
    ...
    if (DEBUG_WHATSAPP) console.log(`[WhatsApp Send] [${requestId}]  Response sent to client`, {
      timestamp: new Date().toISOString(),
    });
  } catch (error: any) {
    const duration = Date.now() - startTime;
    console.error(`[WhatsApp Send] [${requestId}] ﯔﯔﯔ ERROR SENDING MESSAGE ﯔﯔﯔ`, {
 ...
 * Get message history
 */
whatsappRouter.get('/messages', async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    if (DEBUG_WHATSAPP) console.log(`[WhatsApp Messages] [${requestId}]  Messages retrieved`, {
    ...
    console.error(`[WhatsApp Messages] [${requestId}] ﯔ Error getting messages`, {
      error: error.message,
      errorStack: error.stack?.substring(0, 500),
      tenantId: req.tenantId,
      timestamp: new Date().toISOString(),
 ...
 */
whatsappRouter.get('/messages/:messageId/status', async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    const messages = statusResult.rows;
 ...
 */
whatsappRouter.get('/messages/:messageId/check-meta', async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    const messages = metaResult.rows;
 ...
 */
whatsappRouter.post('/messages/:messageId/read', async (req: AuthedRequest, res) => {
  try {
    const { messageId } = req.params;
 ...
 */
whatsappRouter.post('/messages/read-all', async (req: AuthedRequest, res) => {
  try {
 ...
 */
whatsappRouter.get('/unread-count', async (req: AuthedRequest, res) => {
  try {
    const whatsappService = getWhatsAppApiService();
 ...
 */
whatsappRouter.get('/unread-conversations', async (req: AuthedRequest, res) => {
  try {
    const whatsappService = getWhatsAppApiService();
 ...
 */
whatsappRouter.post('/send-document', async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    if (DEBUG_WHATSAPP) console.log(`[WhatsApp Send Document] [${requestId}]  Document sent successfully`, {
      messageId: result.messageId,
      duration: `${duration}ms`,
      timestamp: new Date().toISOString(),
    });
    ...
    const duration = Date.now() - startTime;
    console.error(`[WhatsApp Send Document] [${requestId}] ﯔ Error sending document`, {
      error: error.message,
      errorStack: error.stack?.substring(0, 1000),
      duration: `${duration}ms`,
      timestamp: new Date().toISOString(),
