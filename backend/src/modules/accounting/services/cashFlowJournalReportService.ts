/**
 * Cash flow statement (IAS 7 direct method) from journal_lines on Bank/Cash accounts.
 * Dimension scope resolves project/building from journal line, entry header, and source document.
 */
...
} from '../../../financial/dimensionScope.js';
import {
  addDaysYmd,
...
function scopedJournalSql(
  scope: ReturnType<typeof scopeFromReportFilters>,
  params: unknown[],
  rbacScopeCtx?: DataScopeEnforcementContext
) {
  if (!isDimensionScopeActive(scope)) {
    const rbacConditions: string[] = [];
    appendFinancialRbacScopeSql(rbacConditions, params, rbacScopeCtx, {
      project: 'jl.project_id',
      property: 'jl.property_id',
    });
    const rbacFilter = rbacConditions.length ? ` AND ${rbacConditions.join(' AND ')}` : '';
    return { joins: '', filter: rbacFilter };
  }
  const rbacConditions: string[] = [];
  appendFinancialRbacScopeSql(rbacConditions, params, rbacScopeCtx, {
    project: 'jl.project_id',
  ...
  const rbacFilter = rbacConditions.length ? ` AND ${rbacConditions.join(' AND ')}` : '';
  return {
    joins: buildCashFlowDimensionJoins('je'),
    filter: buildCashFlowDimensionSql(scope, params, { lineAlias: 'jl', entryAlias: 'je' }) + rbacFilter,
  };
}
...
async function sumCashBalanceThrough(
  ...
  const { joins, filter } = scopedJournalSql(scope, params, rbacScopeCtx);
...
export async function getCashFlowReportFromJournal(
  ...
  const [opening_cash, closing_cash] = await Promise.all([
    sumCashBalanceThrough(client, tenantId, dayBeforeFrom, scope, rbacScopeCtx),
    sumCashBalanceThrough(client, tenantId, to, scope, rbacScopeCtx),
  ]);
