...
function accessContext(req: AuthedRequest) {
  return { userId: req.userId ?? null, role: req.role ?? null };
}

installmentPlansRouter.get('/installment-plans/approvers', async (req: AuthedRequest, res) => {
  ...
  try {
    const rows = await withTransaction((client) => listMarketingPlanApprovers(client, tenantId));
...
installmentPlansRouter.get('/installment-plans', async (req: AuthedRequest, res) => {
  ...
  try {
    const rows = await withTransaction((client) =>
      listInstallmentPlans(client, tenantId, { projectId }, accessContext(req))
    );
...
installmentPlansRouter.get('/installment-plans/:id', async (req: AuthedRequest, res) => {
  ...
  try {
    const row = await withTransaction((client) =>
      getInstallmentPlanById(client, tenantId, id, accessContext(req))
    );
    if (!row) {
...
installmentPlansRouter.post('/installment-plans', async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    if (msg.includes('Only company admins') || msg.includes('You can only')) {
      sendFailure(res, 403, 'FORBIDDEN', msg);
      return;
    }
    if (msg.includes('Approver must be')) {
...
installmentPlansRouter.put('/installment-plans/:id', async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    const result = await withTransaction((client) =>
      upsertInstallmentPlan(client, tenantId, body, req.userId ?? null, req.role ?? null)
    );
    if (result.conflict) {
      sendFailure(res, 409, 'CONFLICT', 'Record was modified by another user', { serverVersion: result.row.version });
      return;
