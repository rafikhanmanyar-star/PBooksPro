...
import { getBmAnalysisReportJson } from '../services/bmAnalysisReportService.js';
import { memoryCacheGet, memoryCacheSet } from '../../../utils/memoryCache.js';

export const bmAnalysisRouter = Router();

const TTL_MS = 120_000;

/** Server-side BM (building maintenance) analysis report (LAN mode). */
bmAnalysisRouter.get('/reports/bm-analysis', async (req: AuthedRequest, res) => {
  ...
  const cacheKey = `bm_analysis:${tenantId}:${startDate}:${endDate}:${buildingId}:${search}:${sortKey}:${sortDirRaw}`;
  ...
  try {
    ...
    try {
      const payload = await getBmAnalysisReportJson(client, tenantId, {
        startDate,
        endDate,
        buildingId,
        search,
