...
export function rowToPayslipApi(row: PayslipRow): Record<string, unknown> {
  const base: Record<string, unknown> = {
    ...
    working_days: row.working_days != null ? numStr(row.working_days) : undefined,
    present_days: row.present_days != null ? numStr(row.present_days) : undefined,
    leave_days: row.leave_days != null ? numStr(row.leave_days) : undefined,
    paid_leave_days: row.paid_leave_days != null ? numStr(row.paid_leave_days) : undefined,
    unpaid_leave_days: row.unpaid_leave_days != null ? numStr(row.unpaid_leave_days) : undefined,
    absent_days: row.absent_days != null ? numStr(row.absent_days) : undefined,
    half_days: row.half_days != null ? numStr(row.half_days) : undefined,
    lop_days: row.lop_days != null ? numStr(row.lop_days) : undefined,
    lop_deduction: numStr(row.lop_deduction ?? '0'),
    adjusted_basic: row.adjusted_basic != null ? numStr(row.adjusted_basic) : undefined,
    attendance_summary_snapshot: row.attendance_summary_snapshot ?? undefined,
    is_paid: row.is_paid,
