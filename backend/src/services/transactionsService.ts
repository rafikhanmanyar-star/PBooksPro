...
function normLink(v: string | null | undefined): string {
  return v == null ? '' : String(v).trim();
}

function assertVendorSettlementCashMirrorUnchanged(before: TransactionRow | null, next: PickedTransactionBody): void {
  ...
  const protectedLinks: Array<[string, string | null, string | null | undefined]> = [
    ['type', before.type, next.type],
    ['bill', before.bill_id, next.bill_id],
    ['project', before.project_id, next.project_id],
    ['vendor', before.vendor_id, next.vendor_id],
    ['category', before.category_id, next.category_id],
  ];
  const changed = protectedLinks.find(([, prev, incoming]) => normLink(prev) !== normLink(incoming));
  if (changed) {
    throw new Error(
      'This payment mirrors a supplier prepaid settlement journal. To change settlement accounting links, use Reverse settlement on the bill payment row, then record the settlement again.'
    );
  }
}
...
export async function updateTransaction(
  ...
  assertVendorSettlementCashMirrorUnchanged(before, p);
...
export async function upsertTransaction(
  ...
  assertVendorSettlementCashMirrorUnchanged(existing, p);
