...
import { emitWhatsAppMessageSent, emitWhatsAppMessageReceived, emitWhatsAppMessageStatus } from '../../core/realtime.js';
...
export class WhatsAppApiService {
  ...
  async getConfig(tenantId: string): Promise<WhatsAppConfig | null> {
    try {
      const configs = await dbQuery<WhatsAppConfig>(
  ...
  private async metaRequest(
    ...
    const url = `${this.apiBaseUrl}/${this.apiVersion}${path.startsWith('/') ? path : `/${path}`}`;
    ...
    if (!res.ok) {
      ...
      const err = new Error(errBody.error?.message || res.statusText) as Error & {
  ...
  async sendTextMessage(
    ...
    if (!config) {
      console.error(`[WhatsApp API Service] [${requestId}] ÔØî NO CONFIGURATION FOUND FOR TENANT`, {
        tenantId,
    ...
    try {
      ...
      const responseData = await this.metaRequest(config, 'POST', apiUrl, requestPayload);
      const messages = responseData.messages as Array<{ id: string }> | undefined;
      const metaError = responseData.error as
        | { code?: unknown; type?: unknown; message?: unknown; error_subcode?: unknown }
        | undefined;

      const fullResponseData = JSON.stringify(responseData);

      if (metaError) {
        console.error(`[WhatsApp API Service] [${requestId}] Meta API returned error in response`, {
          errorCode: metaError.code,
          errorType: metaError.type,
          errorMessage: metaError.message,
          errorSubcode: metaError.error_subcode,
          fullError: JSON.stringify(metaError),
          timestamp: new Date().toISOString(),
        });
      }
      ...
      emitWhatsAppMessageSent(tenantId, {
        id: dbMessageId,
        tenantId,
        contactId,
  ...
  async sendDocumentMessage(
    ...
    try {
      ...
      (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${requestId}] Ô£àÔ£àÔ£à DOCUMENT MESSAGE SENT SUCCESSFULLY Ô£àÔ£àÔ£à`, {
        messageId,
        wamId: messageId,
      ...
      const duration = Date.now() - startTime;
      console.error(`[WhatsApp API Service] [${requestId}] ÔØîÔØîÔØî ERROR SENDING DOCUMENT MESSAGE ÔØîÔØîÔØî`, {
        error: error.message,
  ...
  async getVerifyToken(tenantId: string): Promise<string | null> {
    try {
      const configs = await dbQuery<{ verify_token: string }>(
          ...
          if (entry.changes && Array.isArray(entry.changes)) {
            for (const change of entry.changes) {
              ...
              if (change.value && change.value.messages) {
                ...
                (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${webhookId}] ­ƒô¿­ƒô¿­ƒô¿ INCOMING MESSAGES DETECTED ­ƒô¿­ƒô¿­ƒô¿`, {
                ...
                for (let i = 0; i < messages.length; i++) {
                  ...
                  try {
                    ...
                    (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${webhookId}] Ô£à Completed processing message ${i + 1}/${messages.length}`, {
                      timestamp: new Date().toISOString(),
                    });
                  } catch (messageError: any) {
                    // Log error but continue processing other messages
                    console.error(`[WhatsApp API Service] [${webhookId}] ÔØî Error processing message ${i + 1}/${messages.length}`, {
                ...
                (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${webhookId}] Ô£àÔ£àÔ£à ALL ${messages.length} INCOMING MESSAGE(S) PROCESSED Ô£àÔ£àÔ£à`, {
                  timestamp: new Date().toISOString(),
                });
              }
  ...
  private async processIncomingMessage(
    ...
    try {
      ...
      const exactMatch = await dbQuery<{ id: string; name: string }>(
      ...
      if (exactMatch.length > 0) {
        ...
        const rawMatch = await dbQuery<{ id: string; name: string }>(
          'SELECT id, name FROM contacts WHERE tenant_id = $1 AND contact_no = $2',
          [tenantId, rawFrom]
        );
      ...
      if (!contactId && !contactName) {
        const allContacts = await dbQuery<{ id: string; name: string; contact_no: string | null }>(
      ...
      if (message.image) {
        ...
        mediaType = 'image';
        (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${messageId}] ­ƒôÀ Image message detected`, {
          imageId: mediaUrl,
        ...
        mediaType = 'video';
        (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${messageId}] ­ƒÄÑ Video message detected`, {
          videoId: mediaUrl,
        ...
        mediaType = 'document';
        (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${messageId}] ­ƒôä Document message detected`, {
          documentId: mediaUrl,
        ...
        mediaType = 'audio';
        (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${messageId}] ­ƒÄÁ Audio message detected`, {
          audioId: mediaUrl,
        });
      } else if (message.sticker) {
        mediaUrl = message.sticker.id;
        mediaType = 'sticker';
        (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${messageId}] ­ƒÿÇ Sticker message detected`, {
          stickerId: mediaUrl,
        });
      }
      ...
      const existingMessage = await dbQuery<{ id: string }>(
      ...
      if (existingMessage.length > 0) {
        (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${messageId}] ÔÜá´©Å Duplicate message detected, skipping insert`, {
          metaMessageId,
      ...
      try {
        const menuConfigRows = await dbQuery<{ value: any }>(
      ...
      try {
        ...
        if (dbError.code === '23505' || dbError.message?.includes('UNIQUE constraint') || dbError.message?.includes('duplicate key')) {
          (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${messageId}] ÔÜá´©Å Duplicate message detected (database constraint), skipping`, {
            metaMessageId,
            phoneNumber: phoneNumber.substring(0, 5) + '***',
      ...
      const duration = Date.now() - startTime;
      (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${messageId}] Ô£àÔ£àÔ£à INCOMING MESSAGE SAVED TO DATABASE Ô£àÔ£àÔ£à`, {
        dbMessageId,
        phoneNumber: phoneNumber.substring(0, 5) + '***',
      ...
      emitWhatsAppMessageReceived(tenantId, {
        id: dbMessageId,
        tenantId,
        contactId,
        phoneNumber,
      ...
      const duration = Date.now() - startTime;
      console.error(`[WhatsApp API Service] [${messageId}] ÔØîÔØîÔØî ERROR PROCESSING INCOMING MESSAGE ÔØîÔØîÔØî`, {
        error: error.message,
        errorStack: error.stack?.substring(0, 1000),
        tenantId,
  ...
  private async processMessageStatus(tenantId: string, status: any): Promise<void> {
    ...
    try {
      ...
      const existingMessages = await dbQuery<{
        id: string;
        status: string;
      ...
      (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${statusId}] Ô£àÔ£àÔ£à MESSAGE STATUS UPDATED IN DATABASE Ô£àÔ£àÔ£à`, {
      ...
      if (mappedStatus === 'delivered') {
        (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${statusId}] ­ƒô▒ MESSAGE DELIVERED TO RECIPIENT`, {
          wamId: messageId,
          note: 'Message was successfully delivered to recipient\'s device',
          timestamp: new Date().toISOString(),
        });
      } else if (mappedStatus === 'read') {
        (DEBUG_WHATSAPP ? console.log : () => {})(`[WhatsApp API Service] [${statusId}] ­ƒæü´©Å MESSAGE READ BY RECIPIENT`, {
          wamId: messageId,
          note: 'Recipient has opened and read the message',
          timestamp: new Date().toISOString(),
        });
      } else if (mappedStatus === 'failed') {
        console.error(`[WhatsApp API Service] [${statusId}] ÔØî MESSAGE DELIVERY FAILED`, {
          wamId: messageId,
      ...
      emitWhatsAppMessageStatus(tenantId, {
        messageId,
        wamId: messageId,
        status: mappedStatus,
        timestamp: new Date(),
  ...
  async getMessages(
    ...
    try {
      ...
      const messages = await dbQuery<WhatsAppMessage>(query, params);
  ...
  async testConnection(
    ...
    try {
      ...
      const responseData = await this.metaRequest(config, 'GET', testUrl);
  ...
  private async handleAutoReplyMenu(
    ...
    try {
      ...
      const configRows = await dbQuery<{ value: any }>(
        `SELECT value FROM app_settings WHERE tenant_id = $1 AND key = 'whatsapp_auto_menu'`,
        [tenantId]
      );
