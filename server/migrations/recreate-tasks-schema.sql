-- Migration: Recreate Tasks Management Schema
-- This script DROPS existing task tables and recreates them from scratch
-- Run this in DBeaver to fix the missing columns issue
...
CREATE TABLE task_performance_scores (
    id TEXT PRIMARY KEY,
    tenant_id TEXT NOT NULL,
    user_id TEXT NOT NULL,
...
CREATE TABLE task_performance_config (
    id TEXT PRIMARY KEY,
...
CREATE INDEX idx_tasks_tenant_deadline ON tasks(tenant_id, hard_deadline);

-- Indexes for task_updates table
CREATE INDEX idx_task_updates_task_id ON task_updates(task_id);
CREATE INDEX idx_task_updates_tenant_id ON task_updates(tenant_id);
...
-- Indexes for task_performance_scores table
CREATE INDEX idx_task_performance_scores_tenant_id ON task_performance_scores(tenant_id);
CREATE INDEX idx_task_performance_scores_user_id ON task_performance_scores(user_id);
CREATE INDEX idx_task_performance_scores_period ON task_performance_scores(period_start, period_end);
CREATE INDEX idx_task_performance_scores_tenant_period ON task_performance_scores(tenant_id, period_start, period_end);
...
-- DONE!
-- ============================================================================
-- The tasks schema has been recreated successfully.
-- All old data has been removed. You can now start using the task management system.
