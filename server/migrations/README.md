...
- **`sync-staging-to-production.sql`** — Generated by `npm run sync-staging` to sync staging with production schema. Use only when **staging** is behind **production**. For upgrading **production** from **staging**, use the normal deploy flow (merge → deploy → migrations on startup).
