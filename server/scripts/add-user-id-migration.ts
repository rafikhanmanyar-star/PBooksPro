/**
 * Standalone migration script to add user_id column to transactions table
 * Run this script to add the user_id column to existing databases
 * 
 * Usage:
 *   npx tsx server/scripts/add-user-id-migration.ts
 *   or
 *   node dist/scripts/add-user-id-migration.js
 */
...
async function runUserIdMigration() {
  ...
  try {
    console.log('🔄 Running user_id migration for transactions table...');
    ...
    if (!migrationPath) {
      throw new Error(`Could not find add-user-id-to-transactions.sql. Tried: ${possiblePaths.join(', ')}`);
    }
    ...
    console.log('   The transactions table now has a user_id column.');
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.includes('add-user-id-migration')) {
  runUserIdMigration()
    .then(() => {
