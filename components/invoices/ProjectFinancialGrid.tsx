...
const ProjectFinancialGrid: React.FC<ProjectFinancialGridProps> = ({
    ...
    const Row = ({ index, style }: { index: number; style: React.CSSProperties; ariaAttributes?: any }) => {
        ...
        if (record.type === 'Invoice') {
            ...
            // Project selling (unit owners): one payment label. Rental/tenant receipts use RentalFinancialGrid (Rent Pmt / Sec Pmt).
            if (isBulk) {
                displayType = 'Bulk Payment';
                typeClass = 'ds-pill-type ds-pill-type-bulk';
            } else {
                displayType = 'Payment';
                typeClass = 'ds-pill-type ds-pill-type-payment';
            }
        }
