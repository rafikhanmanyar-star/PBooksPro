      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {PRODUCT_TOUR_IDS.map((tourId) => {
          ...
          return (
            <div
              key={tourId}
              className="rounded-xl border border-app-border bg-app-toolbar/30 p-4 flex flex-col gap-3"
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h4 className="font-semibold text-app-text">{tour.title}</h4>
                  <p className="text-xs text-app-muted mt-0.5">{tour.description}</p>
                </div>
                {completed && <CheckCircle2 className="w-5 h-5 text-ds-success shrink-0" aria-hidden />}
                {inProgress && <PauseCircle className="w-5 h-5 text-primary shrink-0" aria-hidden />}
              </div>
              <p className="text-xs text-app-muted">{statusLabel(tourId, progress)}</p>
