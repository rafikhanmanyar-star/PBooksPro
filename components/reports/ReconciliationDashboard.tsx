...
function statusColor(status: CertificationStatus): string {
  switch (status) {
    ...
    case 'critical':
      return 'bg-ds-danger/10 text-ds-danger border-ds-danger/30';
    default:
      return 'bg-app-toolbar text-app-text border-app-border';
  }
}
      ...
      {data && (
        <>
              ...
              {data.checks.map((check) => (
                <li key={check.id} className="px-5 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-app-text">{check.label}</span>
                  </div>
                  <div className="text-xs text-app-muted sm:text-right">
                    {check.expected != null && (
                      <span>
          ...
          {data.missingJournals.length > 0 && (
            <section className="rounded-xl border border-app-border bg-app-card shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-app-border">
                <h3 className="font-semibold text-app-text">Missing journal mirrors</h3>
                <p className="text-xs text-app-muted mt-0.5">
                  Transactions without posted journal entries — run{' '}
                  <code className="bg-app-toolbar px-1 rounded">npm run backfill-transaction-journal --prefix backend</code>
                </p>
              </div>
              <div className="overflow-x-auto max-h-64">
                <table className="w-full text-sm">
                  <thead className="bg-app-table-header text-left text-xs uppercase text-app-muted">
                    <tr>
          ...
          <section className="rounded-xl border border-app-border bg-app-card shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-app-border">
              <h3 className="font-semibold text-app-text">Report source audit</h3>
              <p className="text-xs text-app-muted mt-0.5">Data sources for each financial report</p>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-app-border">
                  {data.reportSources.map((r) => (
                    <tr key={r.reportId}>
                      <td className="px-5 py-2 font-medium text-app-text">{r.reportName}</td>
                      ...
                      <td className="px-5 py-2 text-app-muted text-xs max-w-md">{r.notes}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section className="rounded-xl border border-app-border bg-app-table-header p-5 text-sm text-app-muted">
            <h3 className="font-semibold text-app-text mb-2">Totals (journal-backed)</h3>
