...
const ProjectProfitLossReport: React.FC = () => {
    ...
    const reportData = useMemo<PLData>(() => {
        const {
            ...
            assetSaleRevenue,
            assetSaleCost,
        } = computeProjectProfitLossTotals(state, selectedProjectId, startDate, endDate);
        ...
        const buildHierarchy = (type: TransactionType): ReportRow[] => {
            ...
            const uncKey = type === TransactionType.INCOME ? 'uncategorized_income' : 'uncategorized_expense';
            const uncAmount = categoryAmounts[uncKey] || 0;
            if (uncAmount !== 0) {
                const totalForType = type === TransactionType.INCOME ? totalIncome : totalExpense;
                rows.push({
                    id: uncKey,
                    ...
                    amount: uncAmount,
                    percentage: totalForType > 0 ? (uncAmount / totalForType) * 100 : 0,
                    level: 0,
                    type: 'item'
                });
            }
        ...
        const gainLossFixedAsset = assetSaleRevenue - assetSaleCost;
        if (assetSaleRevenue > 0 || assetSaleCost > 0) {
            const row = {
                id: 'gain-loss-fixed-asset',
                categoryName: gainLossFixedAsset >= 0 ? 'Gain on Sale of Fixed Assets' : 'Loss on Sale of Fixed Assets',
                amount: gainLossFixedAsset >= 0 ? gainLossFixedAsset : -gainLossFixedAsset,
                percentage: (gainLossFixedAsset >= 0 ? totalIncome : totalExpense) > 0
                    ? ((gainLossFixedAsset >= 0 ? gainLossFixedAsset : -gainLossFixedAsset) / (gainLossFixedAsset >= 0 ? totalIncome : totalExpense)) * 100
                    : 0,
                level: 1,
                type: 'item' as const
            };
            if (gainLossFixedAsset >= 0) incomeRows.push(row);
