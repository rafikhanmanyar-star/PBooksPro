...
import {
  ...
  FINANCIAL_ENTITY_PROJECT_PREFIX,
...
export interface ReportStateSlice {
  ...
  properties?: { id: string; buildingId?: string }[];
 ...
 * Resolve buildingId for a transaction from linked invoice, bill, or property when missing on the transaction.

 */

export function resolveBuildingIdForTransaction(
  ...
  if (!buildingId && tx.billId) {

    const bill = state.bills.find((b) => b.id === tx.billId);

    if (bill) buildingId = bill.buildingId;

  }

  if (!buildingId && tx.invoiceId) {

    const inv = state.invoices.find((i) => i.id === tx.invoiceId);

    if (inv) buildingId = inv.buildingId;

  }

  if (!buildingId && tx.propertyId && state.properties?.length) {
 ...
 * Transactions for entity-scoped Trial Balance (same resolution and void rules as P&amp;L).

 */

export function filterTransactionsForTrialBalanceEntityScope(
  ...
  const scope = entityScopeFromFilterId(entityFilterId);

  if (scopeIsConsolidated(scope)) {
  ...
  return transactions.filter((t) => {
    ...
    return transactionMatchesFinancialEntityScope(t, state, scope);
...
/** @deprecated Use filterTransactionsForTrialBalanceEntityScope */
...
export function filterTransactionsForTrialBalanceProjectScope(
  ...
  const filterId =
    ...
    projectId === FINANCIAL_ENTITY_FILTER_ALL
      ...
      : `${FINANCIAL_ENTITY_PROJECT_PREFIX}${projectId}`;

  return filterTransactionsForTrialBalanceEntityScope(transactions, filterId, state);
