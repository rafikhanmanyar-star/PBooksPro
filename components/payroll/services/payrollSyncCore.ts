/**
 * Core payroll sync — pulls API data into localStorage. No dedupe / freshness.
 */
...
export async function syncPayrollFromServerCore(
  tenantId: string,
  options?: SyncPayrollFromServerOptions
): Promise<void> {
  if (!tenantId) return;
