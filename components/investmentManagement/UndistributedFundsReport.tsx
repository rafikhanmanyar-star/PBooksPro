    ...
    return (
        <div className="flex flex-col h-full space-y-4 min-h-0 bg-app-bg">
            ...
            <div className="flex flex-col gap-3 shrink-0">
                <div>
                    <h3 className="text-xl font-bold text-app-text">Undistributed funds</h3>
            ...
            <div className="flex-1 min-h-0 overflow-auto border border-app-border rounded-lg bg-app-card">
                <div id="undistributed-funds-print" className="min-w-0 p-4">
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-sm">
                            <thead className="bg-app-table-header sticky top-0 z-10">
                                <tr className="text-left text-app-muted">
                                    ...
                                    <th className="px-3 py-2 font-semibold text-right whitespace-nowrap min-w-[11rem]">
                                        ...
                                        <span className="block text-[10px] font-normal text-app-muted tracking-normal">
                            ...
                            <tbody className="divide-y divide-app-border">
                                {rows.length === 0 ? (
                                    <tr>
                                        <td colSpan={10} className="px-3 py-10 text-center text-app-muted">
                                    ...
                                    rows.map((r, idx) => (
                                        <tr key={r.projectId} className="hover:bg-app-table-hover">
                                            <td className="px-2 py-2 text-right tabular-nums text-app-muted">
                                                {idx + 1}
                                            </td>
                                            <td className="px-3 py-2 font-medium text-app-text whitespace-nowrap">
                                                {r.projectName}
                                            </td>
                                            <td className="px-3 py-2 text-right tabular-nums text-app-text">{fmtWhole(r.initialInvestment)}</td>
                                            <td className="px-3 py-2 text-right tabular-nums text-app-text">{fmtWhole(r.totalExpense)}</td>
                                            <td className="px-3 py-2 text-right tabular-nums text-ds-success">
                                                {fmtWhole(r.totalRevenue)}
                                            </td>
                                            <td
                                                className={`px-3 py-2 text-right tabular-nums ${
                                                    r.currentProfit >= 0 ? 'text-app-text' : 'text-ds-danger'
                                                }`}
                                            >
                                                {fmtWhole(r.currentProfit)}
                                            </td>
                                            <td className="px-3 py-2 text-right tabular-nums font-medium text-app-text">
                                                {fmtWhole(r.totalEquity)}
                                            </td>
                                            <td className="px-3 py-2 text-right tabular-nums text-ds-success">
                                                {fmtWhole(r.profitDistributed)}
                                            </td>
                                            <td className="px-3 py-2 text-right tabular-nums text-ds-danger">
                                ...
                                {rows.length > 0 && (
                                    <tr className="bg-app-table-header font-semibold border-t-2 border-app-border">
                                        <td className="px-2 py-2 text-right text-app-muted">—</td>
