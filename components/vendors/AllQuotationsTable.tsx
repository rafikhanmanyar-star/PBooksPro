...
const AllQuotationsTable: React.FC<AllQuotationsTableProps> = ({ onEditQuotation }) => {
    ...
    const rows = useMemo((): QuotationRow[] => {
        ...
        return quotations.map((quotation) => {
            ...
            const itemsDetail = itemLines.slice(0, 2).join('; ');
    ...
    }, [state.quotations, state.vendors, state.categories]);
        ...
        <div className="h-full flex flex-col">
            {rows.length === 0 ? (
                ...
                <SmartTable
                    ...
                    getRowId={(r) => r.id}
                    tableHeight={520}
                    searchPlaceholder="Search quotations by vendor, name, date, or category…"
                    showFooterSum
                />
            )}
        </div>
