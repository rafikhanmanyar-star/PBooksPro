...
interface InitializationScreenProps {
    ...
    initProgress: number;
    useFallback?: boolean;
    /** When set (e.g. API load failed in LAN mode), show error instead of rotating features. */
...
const InitializationScreen: React.FC<InitializationScreenProps> = ({ 
    ...
    // Fade out when progress reaches 100% (not when showing a blocking error)
    useEffect(() => {
        if (errorMessage) return;
        if (initProgress >= 100) {
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 text-center">
                ...
                {/* Feature Highlight Section — or API load error */}
                {errorMessage ? (
                    <div className="max-w-lg mx-auto px-4 text-left bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                        <h2 className="text-xl sm:text-2xl font-bold text-white mb-3">
                            Could not load data from the server
                        </h2>
                        <p className="text-sm text-blue-100 mb-2">
                            The app will not use a local database copy in API mode. Check your network, API URL, and that the server is running.
                        </p>
                        <p className="text-xs font-mono text-red-100 bg-black/20 rounded p-3 break-words whitespace-pre-wrap">
                            {errorMessage}
                        </p>
                        {onRetry && (
                            <button
                                type="button"
                                onClick={onRetry}
                                className="mt-4 w-full sm:w-auto px-5 py-2.5 rounded-lg bg-white text-indigo-700 font-semibold hover:bg-blue-50 transition-colors"
                            >
                                Retry (reload page)
                            </button>
                        )}
            ...
            {/* Progress Bar Section at Bottom — hide shimmer when error */}
                    ...
                    <div className="w-full h-2 bg-white/20 rounded-full overflow-hidden shadow-inner">
                        <div 
                            ...
                            style={{ 
                                ...
                                animation: errorMessage ? undefined : 'progressShimmer 2s ease-in-out infinite'
                            }}
                        />
                    </div>
                    <div className="mt-3 text-center">
                        <span className="text-sm sm:text-base font-semibold text-white/90">
                            {errorMessage ? initMessage : `${Math.round(initProgress)}%`}
                        </span>
                    </div>
                </div>
            </div>
