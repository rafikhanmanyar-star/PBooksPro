...
export default function VoiceCapturePanel({ onParsed, onError, disabled }: Props) {
  ...
  useEffect(() => {
    ...
    void getMicrophonePermissionState().then(setMicState);
    return () => {
  ...
  const processTranscript = useCallback(
    (text: string) => {
      ...
      if (!parsed) {
        onError('Could not detect amount. Try: "Paid 5000 to vendor for cement at Project Alpha"');
        return;
      }
      onParsed(parsed);
    },
  ...
  const startRecognition = useCallback(() => {
    ...
    recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
      ...
      if (event.error === 'not-allowed' || event.error === 'service-not-allowed') {
        onError('Microphone access denied. Enable the microphone in device Settings for PBooks Pro.');
        setMicState('denied');
        return;
      }
  ...
  const startListening = useCallback(async () => {
    ...
    if (!mic.ok) {
      onError(mic.reason);
      setMicState(await getMicrophonePermissionState());
      return;
    }

    setMicState('granted');
    startRecognition();
  }, [disabled, listening, onError, startRecognition]);

  if (!supported) {
    return (
      <div className="qc-voice-panel rounded-2xl border border-app-border bg-app-card p-4">
        <p className="text-sm text-app-muted">
          Voice capture requires Chrome or Safari on a device with a microphone.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="qc-voice-panel rounded-2xl border border-blue-200/60 dark:border-blue-500/30 bg-blue-50/80 dark:bg-blue-950/20 p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-app-muted mt-0.5 leading-snug">
              Try saying: &quot;Paid 5000 to vendor for cement at Project Alpha&quot;
            </p>
            {lastTranscript && (
        ...
        {micState === 'denied' && !listening && (
          <p className="text-xs text-amber-700 dark:text-amber-400 mt-2 text-center">
            Microphone blocked — enable it in device Settings for PBooks Pro.
          </p>
        )}
    </div>
  );
