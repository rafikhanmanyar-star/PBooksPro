/**
 * Shared money / ratio formatting for analytics modules (project profitability, fund availability, etc.).
 */
...
export function formatCompactMoney(value: number, currency: string = DEFAULT_CURRENCY): string {
  const sign = value < 0 ? '-' : '';
...
export function formatFullMoney(value: number, currency: string = DEFAULT_CURRENCY): string {
