...
$latestYmlFound = Get-ChildItem -Path "release" -Filter "latest.yml" -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
$latestYml = if ($latestYmlFound) { $latestYmlFound.FullName } else { "release\latest.yml" }
