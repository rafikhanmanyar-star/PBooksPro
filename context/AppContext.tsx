...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // 4. Listen for save-database-now message from Electron (critical for data persistence on close)
    // This must be after state is defined
    useEffect(() => {
        const electronAPI = (window as any).electronAPI;
        if (typeof window !== 'undefined' && electronAPI?.onSaveDatabaseNow) {
            const cleanup = electronAPI.onSaveDatabaseNow(async () => {
                ...
                try {
                    ...
                    try {
                        ...
                        await getErrorLogger().logError(initError instanceof Error ? initError : new Error(String(initError)), {
                            ...
                            operation: 'save_before_close_init'
                        });
                        throw initError;
                    }
                    ...
                    try {
                        ...
                        console.error('❌ State save failed:', saveError);
                        await getErrorLogger().logError(saveError instanceof Error ? saveError : new Error(String(saveError)), {
                            errorType: 'database_error',
                            operation: 'save_before_close_state'
                        });
                        throw saveError;
                    }
                    ...
                    try {
                        ...
                        console.log('✅ Database saved successfully before close');
                        console.log(`📊 Saved state: ${state.contacts.length} contacts, ${state.transactions.length} transactions, ${state.invoices.length} invoices`);
                    } catch (persistError) {
                        console.error('❌ Database persistence failed:', persistError);
                        await getErrorLogger().logError(persistError instanceof Error ? persistError : new Error(String(persistError)), {
                            errorType: 'filesystem_error',
                            operation: 'save_before_close_persist'
                        });
