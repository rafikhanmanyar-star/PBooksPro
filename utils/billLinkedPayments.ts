...
/** Sum of ledger amounts linked to a bill (INCOME or EXPENSE with billId — matches applyTransactionEffect bill paid math). */
export function sumExpenseLinkedToBill(transactions: Transaction[], billId: string): number {
    const bid = String(billId);
    return transactions
        .filter((tx) => {
            if (tx.type !== TransactionType.EXPENSE && tx.type !== TransactionType.INCOME) return false;
