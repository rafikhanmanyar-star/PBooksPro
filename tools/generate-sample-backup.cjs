...
function generateSampleDatabase() {
    ...
    const insertUser = db.prepare("INSERT INTO users (id, username, name, role, password, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
    
    // Create default admin user
    console.log('👤 Creating default admin user...');
    insertUser.run('sys-admin', 'admin', 'Administrator', 'Admin', '', now, now);
    console.log('✅ Default admin user created (username: admin, no password)');
    ...
    // Create equity accounts for investors (one per investor)
    console.log('💰 Creating equity accounts for investors...');
    const equityAccountIds = [];
    contactIds.investors.forEach((investorId, index) => {
        const equityAccountId = generateId();
        equityAccountIds.push(equityAccountId);
        const investorName = `Investor ${index + 1}`; // We'll update this with actual name after contacts are created
        insertAccount.run(equityAccountId, `${investorName} Equity`, 'Equity', 0, 0, now, now);
    });
    ...
    // Update equity account names with actual investor names
    console.log('💰 Updating equity account names with investor names...');
    equityAccountIds.forEach((equityAccountId, index) => {
        db.prepare("UPDATE accounts SET name = ? WHERE id = ?").run(`${investorNames[index]} Equity`, equityAccountId);
    });
    ...
    // Investment transactions - Create equity investments for each investor in each project
    console.log('💼 Creating investor equity investments...');
    contactIds.investors.forEach((investorId, investorIndex) => {
        const equityAccountId = equityAccountIds[investorIndex];
        
        // Each investor invests in multiple projects (but not all)
        const projectsToInvestIn = projectIds.filter(() => Math.random() > 0.2); // 80% chance per project
        if (projectsToInvestIn.length === 0) {
            // Ensure at least one investment
            projectsToInvestIn.push(randomElement(projectIds));
        }
        
        projectsToInvestIn.forEach((projectId) => {
            if (transactionCount < targetTransactions) {
                const transactionId = generateId();
                transactionIds.push(transactionId);
                
                // Investment amount varies by project
                const investmentAmount = randomFloat(2000000, 15000000);
                const transactionDate = addMonths(new Date(), -randomInt(1, 18));
                
                // Transfer from bank account to equity account (this represents capital contribution)
                const fromBankAccount = accountIds[0]; // Main Bank Account
                ...
                insertTransaction.run(
                    ...
                    `Capital investment from ${investorNames[investorIndex]} in ${projectNames[projectIds.indexOf(projectId)]}`, 
                    fromBankAccount, 
                    fromBankAccount, 
                    equityAccountId, 
                    null, 
        ...
        // Loan transactions (some investors may have loans)
        ...
        // Additional transfer transactions between accounts
        if (Math.random() > 0.7 && transactionCount < targetTransactions) {
            const transactionId = generateId();
            transactionIds.push(transactionId);
    ...
    console.log(`   - Accounts: ${accountIds.length + equityAccountIds.length} (${accountIds.length} regular + ${equityAccountIds.length} equity)`);
    console.log(`   - Contacts: ${Object.values(contactIds).flat().length}`);
    console.log(`   - Investors: ${contactIds.investors.length} with equity accounts`);
