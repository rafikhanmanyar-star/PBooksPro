/**
 * Upload PBooks Pro Client installer(s) to the GitHub release for the current package.json version.
 * Run after the main desktop `electron-builder --publish always` so the same release tag
 * Also uploads release-api-client/latest.yml as client.yml so electron-updater (channel client)
 * can resolve the Client NSIS installer + blockmap for differential updates.
 *
...
async function main() {
  if (!token) {
    console.warn(
      '[upload-api-client] GH_TOKEN / GITHUB_TOKEN not set — skipping client upload (installer is still in release-api-client/).'
    );
    process.exit(0);
  }
  ...
  if (!slug) {
    console.error('[upload-api-client] Could not parse repository from package.json');
    process.exit(1);
  }

  const tag = `v${version}`;
  const releaseDir = path.join(root, 'release-api-client');
  const base = `PBooks-Pro-Client-Setup-${version}`;
  const exe = path.join(releaseDir, `${base}.exe`);
  const blockmap = path.join(releaseDir, `${base}.exe.blockmap`);

  if (!fs.existsSync(exe)) {
    console.error('[upload-api-client] Missing:', exe);
    process.exit(1);
  }

  console.log(`[upload-api-client] Fetching release ${tag}…`);
  let release;
  try {
    ...
    console.error(
      '[upload-api-client] Release not found:',
      e.message,
      '\n(Run main app publish first so the tag exists.)'
    );
    process.exit(1);
  }
  ...
  if (fs.existsSync(latestYml)) uploads.push({ name: 'client.yml', file: latestYml });

  for (const { name } of uploads) {
    const existing = (release.assets || []).find((a) => a.name === name);
    if (existing) {
      console.log(`[upload-api-client] Replacing asset: ${name}`);
      await deleteAsset(slug.owner, slug.repo, existing.id);
    }
  }

  for (const { name, file } of uploads) {
    console.log(`[upload-api-client] Uploading ${name}…`);
    await uploadAsset(slug.owner, slug.repo, rid, name, file);
  }

  console.log('[upload-api-client] Done. Client installer + update channel attached to', tag);
}

main().catch((e) => {
  console.error('[upload-api-client]', e);
  process.exit(1);
});
